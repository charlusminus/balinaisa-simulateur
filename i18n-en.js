/* Balinaisa.ai, dictionnaire FR -> EN.

   SOURCE DE VERITE UNIQUE des traductions : tools/build-en.js le lit pour generer les pages
   de /en/, tools/check-i18n.js controle qu'aucune cle n'est orpheline. Ne jamais traduire
   ailleurs.

   POURQUOI CE FICHIER EXISTE (14/09/2026)
   Le dictionnaire vivait dans i18n.js, que TOUTES les pages chargent. Or il ne sert que sur
   /en/ : tr() rend sa chaine telle quelle des que la langue n'est pas l'anglais, et la boucle
   de rattrapage de applyDOM sort avant d'avoir commence. L'accueil francais telechargeait donc
   47 Ko (17 Ko compresses) de traductions qu'il n'ouvrait jamais, sur le chemin d'un simulateur
   d'abord mobile. Sorti ici, il n'est charge que par les pages de /en/, ou il sert deux fois :
   window.i18n.t() pour les chaines que simulator.js fabrique a l'execution, et le filet de
   applyDOM si le generateur a rate un noeud.

   ORDRE DE CHARGEMENT : les pages de /en/ chargent ce fichier JUSTE AVANT i18n.js. Les deux
   sont en defer, et les scripts en defer s'executent dans l'ordre du document : window.__I18N_EN
   existe donc quand i18n.js le lit. tools/build-en.js insere la balise, il n'y a rien a ecrire
   a la main. Une page qui l'oublierait ne planterait pas, i18n.js retombe sur un dico vide.

   FORME DU FICHIER : les deux litteraux gardent leurs declarations, `var EN` et `var metaEN`
   suivies d'une accolade. tools/i18n-dict.js les repere par ces chaines EXACTES avant
   d'evaluer le litteral ; les renommer casserait d'un coup le generateur et son controle CI.
   Pour la meme raison ce commentaire ne les ecrit PAS en entier, accolade comprise : indexOf
   tomberait dessus avant la vraie declaration, et le lecteur chercherait son accolade
   fermante au milieu d'une phrase. */
window.__I18N_EN = (function () {
  var EN = {
    // ---- SEO / meta ----
    ...
    ": pas de pixel de régie, pas de constitution d'audience publicitaire, pas de revente de données. Voici l'inventaire de ce qui est déposé ou lu sur votre appareil. Les trois premiers éléments relèvent du fonctionnement strictement nécessaire du service ou de sa sécurité :": ": no ad-network pixel, no advertising audience building, no resale of data. Here is the full inventory of what is stored on or read from your device. The first three items are strictly necessary to run the service or to its security:",
  ...
  // Surcharges des <meta> : elles ne passent pas par EN, dont les cles sont des textes rendus.
  var metaEN = {
